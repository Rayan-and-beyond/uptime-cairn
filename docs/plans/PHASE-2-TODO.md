...
**Status: 2026-09-07. 127 of 133, and the reporting subsystem is end to end.** A
...
> ### The rules 1–2 waiver on the expiry calendar
>
> **The certificate and domain expiry calendar was built on 2026-09-07 by an
> agent, under an explicit maintainer waiver of [AGENTS.md](../../AGENTS.md)
> rules 1 and 2** ("No whole features", "No multi-file scaffolding runs"). It
> spans the store filter, the `report.Store` contract, the document model, the
> composed block, the JSON artifact and the template editor — a vertical slice,
> which is exactly the shape those two rules exist to stop. The waiver was given
> in the session that produced it and is recorded here for the same reason the
> rule 8 waiver is: this file is what somebody reads in a year.
>
> **Rule 4 was not waived and was not needed.** The whole contract was already
> frozen — `certificate_expiry` in the sections enum, `ReportDocument.expiries`
> typed, `UpcomingExpiry` schema'd, `/api/v1/expiries` shipped — so the work was
> filling in a written shape rather than inventing one. The one addition is
> `MonitorIDs` on `ExpiryFilter`, which is an internal store type and not on the
> wire.
>
...
> waiver draws below and it is worth as much here. Three boxes are new: the
> calendar itself, its fetch gate, and the reproducibility of `days_remaining`.
...
- [x] **The calendar is read for `custom` reports and no others**, the same gate the incident log runs under and the same reason: it is another read, and the four-reads-whatever-the-scope property the extended load gate measures is worth keeping for the types that make up almost every scheduled report. No other type can name the section that would draw the block, so nothing pays for a block it cannot render. An empty scope does not ask at all — which matters beyond the saved read, because an empty `MonitorIDs` means *every monitor* to the store, and the query would have answered the emptiest possible document with the whole install's calendar
